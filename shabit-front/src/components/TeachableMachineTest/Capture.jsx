...
import {setStream} from '../../store/mediaSlice';
...
const Capture = () => {
  ...
  const init = useCallback(async() => {
      if(videoSetting){
        ...
        dispatch(setStream(webcamRef.current.stream));
    }
        // const streamTmp = await navigator.mediaDevices.getUserMedia(CONSTRAINTS);
        // dispatch(setStream(streamTmp));
      mediaRecorderRef.current = new MediaRecorder(webcamRef.current.stream, {
        mimeType: 'video/webm',
      });
      // dispatch(setMediaRecorder(new MediaRecorder(webcamRef.current.stream, {
      //   mimeType: 'video/webm',
      // })));
  ...
  useEffect(()=>{
    // if(videoSetting && mode==='startLive') ();
